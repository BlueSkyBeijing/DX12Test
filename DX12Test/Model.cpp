...
Model::Model(UINT bufferCount, string name, LONG width, LONG height) : Renderable{ bufferCount, name, width, height }
{
	using PointType = decltype(ModelData::points)::value_type;
	using TransformType = decltype(ModelData::patchesTransforms)::value_type;
	using ColorType = decltype(ModelData::patchesColors)::value_type;

	controlPointsBuffer = teapot_tutorial::createVertexBuffer(device.Get(), ModelData::points, L"control points");
	controlPointsIndexBuffer = teapot_tutorial::createIndexBuffer(device.Get(), ModelData::patches, L"patches");
	...
	controlPointsBufferView.SizeInBytes = static_cast<UINT>(controlPointsBufferView.StrideInBytes * ModelData::points.size());
	...
	controlPointsIndexBufferView.SizeInBytes = static_cast<UINT>(ModelData::patches.size() * sizeof(uint32_t));

	transformsBuffer = teapot_tutorial::createStructuredBuffer(device.Get(), ModelData::patchesTransforms, L"transforms");
	colorsBuffer = teapot_tutorial::createStructuredBuffer(device.Get(), ModelData::patchesColors, L"colors");

	createTransformsAndColorsDescHeap();
	
	teapot_tutorial::createSrv<TransformType>(device.Get(), transformsAndColorsDescHeap.Get(), 0, transformsBuffer.Get(), ModelData::patchesTransforms.size());
	teapot_tutorial::createSrv<ColorType>(device.Get(), transformsAndColorsDescHeap.Get(), 1, colorsBuffer.Get(), ModelData::patchesColors.size());
...
void Model::createTransformsAndColorsDescHeap()
{
...
void Model::createPipelineStateWireframe()
{
	pipelineStateWireframe = createPipelineState(D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_NONE);
	currPipelineState = pipelineStateWireframe;
}

void Model::createPipelineStateSolid()
{
	pipelineStateSolid = createPipelineState(D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_NONE);
}

ComPtr<ID3D12PipelineState> Model::createPipelineState(D3D12_FILL_MODE fillMode, D3D12_CULL_MODE cullMode)
{
